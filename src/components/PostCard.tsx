...
/* ── List variant ── */
function ListCard({ post, locale = "ko" }: { post: PostMeta; locale?: Locale }) {
  const dict = getDictionary(locale);
  return (
    <article className="py-6 border-b border-stone-100 last:border-0">
      <Link href={`/${locale}/blog/${post.slug}`} className="group block">
        <div className="flex items-center gap-2 mb-2">
          <CategoryBadge category={post.category} size="sm" locale={locale} />
          <span className="text-xs text-stone-400">{formatDate(post.date, locale)}</span>
          <span className="text-xs text-stone-200">&middot;</span>
          <span className="text-xs text-stone-400">{dict.post.readingTime(post.readingTime)}</span>
        </div>
...
/* ── Compact variant ── */
function CompactCard({ post, locale = "ko" }: { post: PostMeta; locale?: Locale }) {
  return (
    <Link
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="mt-0.5 text-xs text-stone-400">{formatDate(post.date, locale)}</p>
      </div>
    </Link>
  );
}
...
/* ── Hero variant ── */
function HeroCard({ post, locale = "ko" }: { post: PostMeta; locale?: Locale }) {
  const dict = getDictionary(locale);
  return (
    <Link
      href={`/${locale}/blog/${post.slug}`}
    ...
    >
      <div className="relative h-72 sm:h-80 lg:h-full lg:min-h-[380px] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6">
          <CategoryBadge category={post.category} size="sm" className="mb-3" locale={locale} />
...
/* ── Featured variant ── */
function FeaturedCard({ post, locale = "ko" }: { post: PostMeta; locale?: Locale }) {
  const dict = getDictionary(locale);
  return (
    <Link
      href={`/${locale}/blog/${post.slug}`}
    ...
    >
      ...
      <div className="p-4">
        ...
        <p className="mt-3 text-xs text-stone-400">
          {formatDate(post.date, locale)} &middot; {dict.post.readingTime(post.readingTime)}
        </p>
      </div>
    </Link>
  );
...
/* ── Default variant ── */
function DefaultCard({ post, locale = "ko" }: { post: PostMeta; locale?: Locale }) {
  const dict = getDictionary(locale);
  return (
    <Link
      href={`/${locale}/blog/${post.slug}`}
    ...
    >
      ...
      <div className="p-5">
        ...
        <p className="mt-3 text-xs text-stone-400">{dict.post.readingTime(post.readingTime)}</p>
      </div>
    </Link>
  );
}

export default function PostCard({ post, variant = "default", locale = "ko" }: PostCardProps) {
  if (variant === "list")     return <ListCard     post={post} locale={locale} />;
  if (variant === "compact")  return <CompactCard  post={post} locale={locale} />;
  if (variant === "hero")     return <HeroCard     post={post} locale={locale} />;
  if (variant === "featured") return <FeaturedCard post={post} locale={locale} />;
  return <DefaultCard post={post} locale={locale} />;
}
